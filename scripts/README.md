...
`inspect-batch` processes inputs sequentially to keep memory use predictable for large source videos. The inspector caches checksum metadata, reuses valid partial ffprobe/scene/contact-sheet artifacts after interruptions, and supports `--cancel-file <path>` between expensive stages. Contact-sheet performance can be tuned with `--contact-sheet-columns`, `--contact-sheet-rows`, `--contact-sheet-width`, and `--ffmpeg-threads`. See `docs/catalog/MEDIA_PROCESSING_PERFORMANCE.md` for current inventory size, expected processing time, disk planning, and recovery steps.
