...
| Character video upload/recording | Implemented in `OWNER_REVIEW_DEMO` with browser recording where supported, iPhone/file upload, local validation, frame extraction, retry, and non-image persisted summaries. | Production still requires verified catalog/recommendation flow and deployed persistence. |
| Before/after comparison | Implemented in `OWNER_REVIEW_DEMO` with Video #1, refinement guide, Video #2, score delta, improvement/no-change/regression, and final summary. | Production still requires verified catalog, verified calibration, and real buddy execution. |
| User resemblance rating | Implemented in `OWNER_REVIEW_DEMO` final screen with version preference, 1-10 rating, optional feedback, and separate product-improvement opt-in. | Real buddy acceptance remains unmeasured. |
| Privacy-safe learning loop | Implemented as structured, consent-gated, demo-excluding offline proposal pipeline. | Production optimization still requires consented real outcomes, owner approval, retained-case validation, versioning, and rollback. |
...
| `RESULT_CAPTURE_1` | User uploads or records created-character result. | The first result accepts supported media, checks resolution, obstruction, lighting, blur, and required view coverage. Raw media is not persisted by default. | Implemented in `OWNER_REVIEW_DEMO`; production blocked by verified catalog/deployed persistence. |
| `REFINEMENT_1` | User receives score and evidence-based changes. | Score is 0-100, labeled as build match, not identity probability. Actions reference only verified controls or verified alternate recommendations. | Implemented in `OWNER_REVIEW_DEMO`; production blocked by verified catalog and verified calibration. |
| `RESULT_CAPTURE_2` | User uploads or records revised result. | Same validation as first result; links to the same buddy session and iteration number. | Implemented in `OWNER_REVIEW_DEMO`; production blocked by verified catalog/deployed persistence. |
| `BEFORE_AFTER` | User sees measurable improvement comparison. | Shows first score, second score, delta, key differences, limitations, and whether score reached the configured 90 threshold. | Implemented in `OWNER_REVIEW_DEMO`; real buddy acceptance unmeasured. |
| `USER_RATING` | User rates whether the result looks like them. | Collects rating, optional notes, final selected settings, and consent choice for product-improvement use. | Implemented in `OWNER_REVIEW_DEMO`; structured learning remains consent-gated and demo-excluded. |
