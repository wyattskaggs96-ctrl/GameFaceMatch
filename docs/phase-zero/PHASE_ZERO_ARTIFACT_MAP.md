...
| Appearance menu gap matrix | `data/phase-zero/appearance_menu_gap_matrix.json` and `data/phase-zero/appearance_menu_gap_matrix.csv` | Appearance-menu gap auditor | Regenerate after menu-map, timeline, research-catalog, screenshot/evidence, or capture-request changes. | Menu map, video timeline, head research catalog, additional-attribute catalog, capture requests. | `npm run cf27:appearance-menu-gap-audit` and `npm test -- cf27-appearance-menu-gap-audit` from `web/` | Canonical exact gap matrix; 22 rows, 0 production eligible. |
...
| Recapture request plan | `data/phase-zero/capture_requests.json` and `data/phase-zero/capture_requests.csv` | Human-capture director | Update from current appearance-menu gaps only; do not add generic or assumed captures. | Appearance menu gap matrix and issue tracker. | `npm run cf27:wyatt-recording-script -- --check` and `npm test -- phase-zero-capture-requests` from `web/` | Canonical executable appearance-menu capture queue; 10 sessions. |
| Dependency-test execution report | `data/phase-zero/dependency_tests.research.json` and `data/phase-zero/dependency_tests.research.csv` | Dependency-testing lead | Regenerate from direct evidence whenever new controlled dependency footage is ingested; never mark unexecuted variables as passed. | Environment manifest, creation paths, head research catalog, additional-attribute catalog, body-control catalog, capture requests, evidence manifest. | `npm run phase0:dependency-tests` and `npm test -- cf27-dependency-evidence-tests` from `web/` | Canonical current dependency execution report; 2 variables have research observations and 14 remain blocked/not executed. |
...
| Appearance menu gap matrix | `docs/phase-zero/APPEARANCE_MENU_GAP_MATRIX.md` | Appearance-menu gap auditor | Regenerate with `data/phase-zero/appearance_menu_gap_matrix.*`; do not infer categories from other games. | Appearance menu gap matrix JSON. | `npm test -- cf27-appearance-menu-gap-audit phase-zero-artifact-map` from `web/` | Canonical exact gap matrix; research-only and not production verified. |
| Menu capture gaps | `docs/phase-zero/MENU_CAPTURE_GAPS.md` | Appearance-menu catalog manager | Update when menu recapture needs change. | Appearance menu gap matrix, menu map, issues, capture requests. | `npm test -- phase-zero-artifact-map` from `web/` | Canonical gap summary. |
...
| Dependency-test execution summary | `docs/phase-zero/DEPENDENCY_TEST_EXECUTION.md` | Dependency-testing lead | Regenerate with `data/phase-zero/dependency_tests.research.*`; do not mark blocked variables as passed. | Dependency-test execution report. | `npm run phase0:dependency-tests` and `npm test -- cf27-dependency-evidence-tests` from `web/` | Canonical dependency execution summary; research-only and incomplete. |
| Wyatt recording script | `docs/phase-zero/WYATT_RECORDING_SCRIPT.md` and `docs/phase-zero/WYATT_RECORDING_QUICK_CHECKLIST.md` | Human-capture director | Update from `capture_requests.*` and the appearance-menu gap matrix. | Appearance menu gap matrix and current capture requests. | `npm run cf27:wyatt-recording-script -- --check` and `npm test -- phase-zero-capture-requests` from `web/` | Canonical human recording plan; `docs/phase-zero/WYATT_NEXT_CAPTURE_PLAN.md` is a preserved compatibility pointer. |
