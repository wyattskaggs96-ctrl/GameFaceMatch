...
- Owner Review Demo scan-to-build journey now reaches a complete one-link customer path from invite to scan-complete checkpoint, synthetic recommendation result, exact settings, step-by-step build walkthrough, persisted progress, build-guide-complete handoff, and Prompt 141 result-photo feedback: one required front-view CF27 created-player image, up to two optional three-quarter images, selected recommendation rank, 1-5 resemblance rating, alternate top-three answer, manual-change capture, notes, research-only refinement signal, local resume, and completion summary.
...
- Internal owner trial command center exists at `/owner/trials` for protected owner operations and beta review. It creates opaque invite links, tracks Buddy Trial progress, records owner intervention, shows per-session beta evidence, displays top-three recommendation provenance, selected rank, CF27 result-photo metadata, resemblance rating, tester mismatch notes, deletion state, runtime errors, owner review notes/disposition, aggregate beta metrics, and exports a privacy-safe beta research package without raw face media or browser object URLs.
...
- Ten-user private beta execution: scope is now locked, and the local owner-review customer flow can reach result-photo/feedback submission plus owner review. Durable Vercel deployment, live Supabase private storage/persistence activation, and physical iPhone Safari validation still need implementation/verification before outside testers are invited.
