...
- Phase 0 audit data uses a JSON-serializable domain model in `web/lib/phase-zero/phase-zero-domain.ts` with the shared schema `data/schemas/phase-zero-domain.schema.json`. The model covers audit sessions, evidence, catalog candidates, verification, releases, imports, and manual matching studies without populating real College Football 27 option records.
